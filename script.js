...
async function addComment() {
    ...
    saveNickname(nickname);
    const floor = post.comments.length + 1;
    const comment = {
        id: Date.now(),
        floor: floor,
        userId: generateUserId(),
...
async function deleteComment(postId, commentId) {
    ...
    post.comments.forEach((c, i) => c.floor = i + 1);
...
function renderComments(post) {
    ...
    commentsList.innerHTML = post.comments.map((comment, i) => {
        // 兼容旧评论：有floor用floor，没有用索引+1
        const floor = comment.floor || (i + 1);
